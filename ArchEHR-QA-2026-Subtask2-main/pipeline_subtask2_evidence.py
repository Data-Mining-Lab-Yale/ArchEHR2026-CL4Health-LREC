"""
Subtask 2: Evidence Identification pipeline.
Identifies evidence (sentence IDs) supporting fact-sheet answers from the EHR.

- Few-shot dev examples in prompt (when dev key + XML available).
...
- Optional TTS (test-time scaling): one model runs N times at high temp, vote-aggregate;
  other models run once. Hybrid TTS+ensemble.
